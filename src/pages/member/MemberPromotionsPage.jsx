...
function Promotions() {
  ...
  const loadPersonsAndPromotions = async () => {
    try {
      ...
      // Get person details for each ID
  ...
  const loadPromotions = async (personId) => {
    try {
      ...
      // Enrich with belt and location details
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#1a1a1a] rounded-2xl shadow-2xl p-6 border-2 border-[#3d3d3d]">
        ...
        {/* Person Selector - ALWAYS SHOW if multiple persons */}
        {persons.length > 1 && (
          <div className="mt-6">
                ...
                <button
                  ...
                  className={`px-6 py-3 rounded-lg font-bold transition-all duration-300 ${
                    selectedPerson?.id === person.id
                      ? 'bg-[#ff6d00] text-white scale-105 shadow-lg'
                      : 'bg-[#2e2e2e] text-gray-300 hover:bg-[#3d3d3d] hover:text-white'
                  }`}
                >
        ...
        {/* Show current viewing person even if only one */}
        {selectedPerson && (
      ...
      {selectedPerson && promotions.length > 0 && (
        <div className="bg-gradient-to-r from-[#ff6d00] to-[#e66200] rounded-2xl shadow-2xl p-8 text-center">
          ...
          <div className={`inline-block px-8 py-4 rounded-xl text-3xl font-black ${getBeltColour(promotions[0].belt.primary_colour)}`}>
                ...
                <div className="flex flex-wrap justify-between items-start gap-4">
                  <div className="flex-1 min-w-[200px]">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`px-4 py-2 rounded-lg font-bold ${getBeltColour(promo.belt.primary_colour)}`}>
                        {promo.belt.name}
                      </span>
                      {promo.tabs > 0 && (
